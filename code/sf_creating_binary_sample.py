...
Output Plots 
Plots will be outputted to the plot_path directory. Details of these plots, and model outputs from the first user, can be found at the GitHub: https://github.com/Jesstella/ss21_companion_github_superflares. 
...
binaries_03, binaries_01 = np.split(binary_g_dwarfs, [int(0.76*len(binary_g_dwarfs))])
...
plt.subplot(121) 
plt.hist(10**(binaries_03['logTe']), range=(min(10**binaries_03['logTe']), max(10**binaries_03['logTe'])) , color='#66c2a4', edgecolor='#41ae76', linewidth=3, histtype='stepfilled', label=r'Mass Ratio = 0.3 - 0.95 [$M_{comp}/M_1$] (76% of Sample)') 
plt.hist(10**(binaries_01['logTe']), range=(min(10**binaries_03['logTe']), max(10**binaries_03['logTe'])), color='#8c6bb1', edgecolor='#88419d', linewidth=3, histtype='stepfilled', label=r'Mass Ratio = 0.1 - 0.3 [$M_{comp}/M_1$] (24% of Sample)')
plt.legend(loc=2)
plt.ylabel('N') 
plt.xlabel('Effective Temperature [K]') 
...
# Cutting the sample again into stars with periods less than 5 days and greater than or equal to 5 days 
binaries_03_large_p = binaries_03[binaries_03['periods'] >= 5]
binaries_03_large_p = pd.DataFrame(binaries_03_large_p) 
binaries_03_small_p = binaries_03[binaries_03['periods'] < 5] 
